...
def profile(user_id):
    if int(user_id) == int(current_user.id):
        ...
        return render_template('profile.html', user=current_user, form=_formCreateNewPost, cur_user=True)

    else:
        _user = User.query.get(int(user_id))
        return render_template('profile.html', user=_user, form=None, cur_user=False)


@app.route('/delete/<_user_id>/<post_id>', methods=['POST'])
def delete(_user_id, post_id):
    _post = Posts.query.get(int(post_id))

    database.session.delete(_post)
    database.session.commit()

    return redirect(url_for('profile', user_id=_user_id))


@app.route('/like/<_user_id>/<post_id>', methods=['POST'])
def like(_user_id, post_id):
    _post = Posts.query.get(int(post_id))
    _post.likes += 1
    _post.dislikes = max(0, _post.dislikes - 1)

    database.session.merge(_post)
    database.session.commit()

    return redirect(url_for('profile', user_id=_user_id))

@app.route('/dislike/<_user_id>/<post_id>', methods=['POST'])
def dislike(_user_id, post_id):
    _post = Posts.query.get(int(post_id))
    _post.dislikes += 1
    _post.likes = max(0, _post.likes - 1)
